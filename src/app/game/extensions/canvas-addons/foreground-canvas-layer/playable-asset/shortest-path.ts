...
export class ShortestPath {
  ...
  private static visitedNow(endingPoint: Cell, visited: any) {
    ...
    Object.keys(visited).forEach(visitedCell => {
        if (!visited[visitedCell].checked) {
          ...
          GSM.GridController.getAllNeighbors(visited[visitedCell].cell, GSM.ElevationController.currentElevationLayerIndex).forEach((cell: Cell, index: number) => {
            if (!cell) {
              return;
            }
  ...
  public static verifyClosetLocation(start: Cell, end: Cell): Cell {
    if(this.isBadLocation(end)) { 
      const possibleAlternatives = GSM.GridController.getAllNeighbors(end, GSM.ElevationController.currentElevationLayerIndex).filter(a => !this.isBadLocation(a))
