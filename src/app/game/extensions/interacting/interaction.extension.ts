...
export class InteractionExtension extends Extension{
  ...
  private findAdjacentAsset(event: any): void {
    ...
    const playerSelectionIndicatorRenderer = GSM.RendererController.getRenderer("SelectionIndicatorRenderer") as SelectionIndicatorRenderer
    const interactionIndicatorRenderer = GSM.RendererController.getRenderer("InteractionIndicatorRenderer") as InteractionIndicatorRenderer
    
    if(assets.length === 0) {
      playerSelectionIndicatorRenderer.animate = true
      interactionIndicatorRenderer.enabled = false
      GSM.EventController.objectInteraction.next(null)
    } else {
      interactionIndicatorRenderer.indicatorURL = "assets/images/indicators/interaction2.png"
      interactionIndicatorRenderer.enabled = true
      playerSelectionIndicatorRenderer.animate = false
      const assetBeingInteractedWith = assets.pop().asset
      interactionIndicatorRenderer.asset = GSM.AssetController.selectedAssets[0]
