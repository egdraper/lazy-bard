...
export class AssetRenderer implements Renderer {
  public id: string = "AssetRenderer"
  ...
  public beforeDraw(asset: PlaceableAsset, frame?: number, opacity: number = 1): void {
    ...
    const topAsset = GSM.AssetManager.getTopAssetByCell(asset.anchorCell, [RenderingLayers.AssetLayer, RenderingLayers.ObjectLayer])

    const shadowZ = topAsset ? topAsset.baseZIndex + topAsset.attributes.size.z : 0
    const coveringAsset = GSM.AssetManager.getAssetBlocksCoveringCellAtZ(asset.anchorCell, shadowZ)
