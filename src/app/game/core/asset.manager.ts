...
import { Orientation } from './default-features/orientation/orientation';
...
export class AssetManager {
  ...
  public addAsset(asset: Asset, anchoringCell: Cell, zIndex: number): void {
    ...
    asset.id = `map:${GSM.GridManager.map.id}-cell:${anchoringCell.id}:z${zIndex}:${asset.layer}`;
  ...
  public getAssetByLocation(x: number, y: number, zIndex: number, layer: RenderingLayers): Asset {
    const cell = GSM.GridManager.getCellByLocation(x,y)
  ...
  public getAssetsByCellAtZ(cell: Cell, zIndex: number): {layer: RenderingLayers, asset: Asset}[] {
    const assets = []
    GSM.RendererManager.iterateAllRenderingLayers(layer => {
  ...
  public changeZAxis(direction: 'up' | 'down', asset: Asset): void {
    const upAssets = GSM.AssetManager.getAssetBlocksAtZ(asset.anchorCell, asset.baseZIndex + asset.attributes.size.z)
    const downAssets = GSM.AssetManager.getAssetBlocksAtZ(asset.anchorCell, asset.baseZIndex - 1);

    if (direction === 'up' && upAssets.length === 0) {
      this.updateBlockProperty(asset, asset.anchorCell, ++asset.baseZIndex);
      GSM.EventManager.assetEnteredCell.next({asset: asset, cell: asset.anchorCell})
      return;
    }
  ...
  public getAllAssetBlocksCoveringCell(coveredCell: Cell): AssetBlock[] {
    ...
    GSM.GridManager.iterateYCellsFrom(coveredCell.location.y, coveredCell.location.x, (cell: Cell) => {
      const distanceFromHoveringCell = cell.location.y - coveredCell.location.y
      GSM.AssetManager.getAllAssetBlocksAtCell(cell).forEach(assetBlock => {
  ...
  public getAssetBlocksCoveringCellAtZ(coveredCell: Cell, zIndex: number): AssetBlock[] {
    ...
    GSM.GridManager.iterateYCellsFrom(coveredCell.location.y + 1, coveredCell.location.x, (cell: Cell) => {
      const distanceFromHoveringCell = cell.location.y - coveredCell.location.y
      GSM.AssetManager.getAssetBlocksAtZ(cell, zIndex + distanceFromHoveringCell - 1).forEach(assetBlock => {
        if(assetBlock.obstructed) {
          coveringBlocks.push(assetBlock)
        }
      })  
  ...
  public getAllAssetsCoveringCell(hoveringCell: Cell): Asset[] {
    ...
    return assetBlocks.map(block => GSM.AssetManager.getAssetById(block.ownerAssetId))
  }
 ...
 public getTopAssetCoveringCell(hoveringCell: Cell, excludeLayers: RenderingLayers[] = []): Asset{
    ...
    const assets = blocks.map(block => GSM.AssetManager.getAssetById(block.ownerAssetId))
  ...
  private animateAsset(frame: number): void {
    GSM.AssetManager.iterateAsset((asset: PlaceableAsset) => {
      if (asset.animating) {
  ...
  private updateBlockProperty(asset: Asset, selectedCell: Cell, zIndex: number) {
    ...
    asset.ownedBlockIds = [];
    asset.anchorCell = GSM.GridManager.getCellByLocation(selectedCell.location.x - xOffset, selectedCell.location.y + yOffset)
    ...
    asset.attributes.obstructed.forEach((zIndexSet, zIndexOffset) => {
      ...
      rows.forEach((obstructions: string, yIndex) => {
        obstructions.split('').forEach((blockObstruction, xIndex) => {
          ...
          const cell = GSM.GridManager.getCellByLocation(cellX, cellY);
  ...
  protected setDirectionByKey(keyEvent: KeyboardEvent): void {
    this.selectedAssets.forEach((asset: PlaceableAsset) => {
      if(asset.orientation) {
        ...
        GSM.EventManager.playerOrientationChanged.next({asset: asset, cell: asset.anchorCell})
      }
    })  
  }
}
