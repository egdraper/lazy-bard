...
import { BaseTexture } from "./base-texture.generator"
...
export class TerrainEdgeCalculator {
  public static calculateTerrainEdges(gridAsset: Asset<TerrainTile>, terrainTile: TerrainTile, drawableItem: DrawableTile): TerrainTile {
    const neighboringCells = GSM.CellNeighborsManager.getAllImmediateNeighbors(gridAsset, RenderingLayers.TerrainLayer) as Asset[]
    ...
    if(tile.id === "MidCenter" && drawableItem.backgroundTerrainId) {
      ...
      GSM.ImageManager.addImageBySrcUrl(backgroundTexture.imageUrl)
      
      if(gridAsset.tile.imageUrl === backgroundTexture.imageUrl) {
        tile = gridAsset.tile
      } else {
        tile.imageUrl = backgroundTexture.imageUrl
        tile.drawsWithTop = BaseTexture.getRandomBackgroundTileCoordinates(backgroundTexture)
      }
    } else {
