...
import { TravelPath } from "../travel-path/travel-path"
...
export abstract class Movement {
  ...
  constructor(
    private animation: AssetMovementAnimation,
  ...
  protected checkForFinishLocation(): void {    
    if (this.cellTrackPosX % (GSM.Settings.blockSize) === 0 && this.cellTrackPosY % (GSM.Settings.blockSize) === 0) {
      ...
      if (!this.nextCell || !this.isNextCellPassable()) {
        GSM.EventManager.assetFinishingMovement.next({asset: this.asset, cell: this.previousCell})
        this.endMovement()
       } else {
        this.prepareNextMovement()
      }
  ...
  protected prepareNextMovement(): void {
    ...
    GSM.EventManager.assetEnteredCell.next({asset: this.asset, cell: this.nextCell})
  ...
  protected subscribeToFrameTimer(): void {
    this.movementSubscription = GSM.FrameManager.frameFire.subscribe(frame => { 
  ...
  private isNextCellPassable(): boolean {
    ...
    if(this.nextCell) {
      nextTerrainAsset = GSM.AssetManager.getAsset(this.nextCell, this.asset.baseZIndex - 1, RenderingLayers.TerrainLayer)
      previousTerrainAsset = GSM.AssetManager.getAsset(this.previousCell, this.asset.baseZIndex - 1, RenderingLayers.TerrainLayer)
    } else {
      return false
    }
