...
export class ShortestPath extends TravelPath {
  ...
  public find(start: Cell, end: Cell, asset: PlaceableAsset): Cell[] {
    ...
    if (GSM.AssetManager.isCellBlockObstructed(end, asset.baseZIndex)) {
  ...
  private visitedNow(endingPoint: Cell, visited: any) {
    ...
    Object.keys(visited).forEach(visitedCell => {
        if (!visited[visitedCell].checked) {
          ...
          GSM.CellNeighborsManager.getHorizontalNeighborsCell(visited[visitedCell].cell).forEach((cell: Cell, index: number) => {
            if (!cell) {
              return
            }

            let blockObstructed = GSM.AssetManager.isCellBlockObstructed(cell, this.asset.baseZIndex)
  ...
  public verifyClosetLocation(start: Cell, end: Cell): Cell {
    if(this.isBadLocation(end)) { 
      const possibleAlternatives = GSM.CellNeighborsManager.getHorizontalNeighborsCell(end).filter(a => !this.isBadLocation(a))
  ...
  public isBadLocation(end: Cell): boolean {
    return GSM.AssetManager.isCellBlockObstructed(end, this.asset.baseZIndex)
  }
